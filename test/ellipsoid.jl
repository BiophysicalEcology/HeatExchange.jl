...
ellipsoid_output = DataFrame(CSV.File("$testdir/data/ellipsoid_output.csv"))[:, 2:end]

air_temperature = u"K".(DataFrame(CSV.File("$testdir/data/ellipsoid_air_temperature.csv"))[:, 2] .* u"°C")
wind_speed = (ellipsoid_input.windspd)u"m/s"
relative_humidity = ellipsoid_input.rh/100
P_atmos = 101325.0u"Pa"

ellipsoid_out = DataFrame(ellipsoid_endotherm.(
    air_temperature,
    wind_speed,
    relative_humidity,
    P_atmos; 
    ...
    core_temperature=u"K"((ellipsoid_input.coreT)u"°C"),
...
@testset "ellipsoid model" begin
    ...
    @test u"K".(ellipsoid_out.lower_critical_air_temperature) ≈ (ellipsoid_output.LCT .+ 273.15)u"K" rtol=rtol
    @test u"K".(ellipsoid_out.upper_critical_air_temperature) ≈ (ellipsoid_output.UCT .+ 273.15)u"K" rtol=rtol
