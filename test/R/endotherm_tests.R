...
endoR_input = list(
    TA = 40, # air temperature at local height (°C)
    TAREF = 40, # air temperature at reference height (°C)
    TGRD = 60, # ground temperature (°C)
    ...
    TCONDSB = 35, # surface temperature for conduction (°C)
    KSUB = 2.79, # substrate thermal conductivity (W/m°C)
    TBUSH = 40, # bush temperature (°C)
    ...
    FURWET = 5, # part of the fur/feathers that is wet after rain (%)
    PCTBAREVAP = 5, # surface area for evaporation that is skin, e.g. licking paws (%)
    ...
    # initial conditions
    TS = 37 - 3, # skin temperature (°C)
    TFA = 40, # fur/air interface temperature (°C)
