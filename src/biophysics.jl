...
"""
function ellipsoid_endotherm(; posture=4.5, mass=0.5u"kg", density=1000.0u"kg/m^3", core_temperature=37u"°C",
                   fur_depth=5u"mm", fur_conductivity=0.04u"W/m/K", oxygen_extraction_efficiency=0.2, stress_factor=0.6,
                   air_temperature=20.0u"°C", wind_speed=1.0u"m/s", relative_humidity=50, atmospheric_pressure = 101325.0u"Pa", Q10=3,
                   minimum_metabolic_rate=missing, metabolic_multiplier=1, lethal_desiccation=0.15, f_O2=0.2094)
    ...
    Q_gen_final = max(Q_gen, Q_gen_min)

    O2_consumption_rate = Q_gen_final / 20.1u"J/mL" # TODO make general function to convert metabolic rate to O2
    ρ_vap_c = wet_air_properties(T_c, rh=100.0, P_atmos=P_atmos).ρ_vap
    ρ_vap_f = wet_air_properties(T_f, rh=relative_humidity, P_atmos=P_atmos).ρ_vap
    ...
    Q_evap = max(Q_respiration, (-Q_gen) + Q_gen_min)
...
function insulation_properties(; 
    ...
    # Physical constants
    air_conductivity = dry_air_properties(air_temperature).k_air
