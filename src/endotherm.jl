...
function solve_without_insulation!(T_skin, T_insulation, 
    ...
    (; side, diameter, r_insulation, r_radiation, r_compressed, cd, conduction_fraction, longwave_depth_fraction, convection_enhancement) = geom_vars
    (; fluid_type, T_air, T_substrate, T_bush, T_vegetation, T_lower, T_sky, T_conduction, rh, wind_speed, P_atmos, F_sky, F_ground, F_bush, F_vegetation, Q_solar, fO2, fCO2, fN2) = env_vars
    (; T_core, k_flesh, k_fat, ϵ_body, skin_wetness, insulation_wetness, bare_skin_fraction, eye_fraction, insulation_conductivity) = traits
    ...
    while true
        ...
        for i in 1:20
            (; Q_conv, hc, hd, hd_free) = convection(geometry_out, area_convection, T_air, T_insulation, wind_speed, P_atmos, fluid_type, fO2, fCO2, fN2)
            ...
            T_skin1 = ((4.0 * k_flesh * volume) / (r_skin^2) * T_core) - Q_evap_skin + hc * area_convection * T_air + Q_solar
            T_skin2 = Q_rad1 * T_sky + Q_rad2 * T_bush + Q_rad3 * T_vegetation + Q_rad4 * T_lower
            T_skin3 = ((4.0 * k_flesh * volume) / (r_skin^2)) + hc * area_convection + Q_rad1 + Q_rad2 + Q_rad3 + Q_rad4
            ...
            if ΔT_skin < tolerance_simusol
                ...
                success = true
                return (; T_insulation, T_skin = T_skin_calc, Q_convection, Q_conduction, Q_gen_net, Q_evap_skin, Q_solar, Q_rad_sky, Q_rad_bush, Q_rad_vegetation, Q_rad_ground, Q_evap_insulation, success, ntry)
            else
                T_skin = T_skin_calc
                ...
                if ntry == 101
                    if tolerance_simusol <= 0.001u"K"
                        ...
                        success = false
                        Q_gen_net = 0.0u"W"
                        return (; T_insulation, T_skin_mean, Q_convection, Q_conduction, Q_gen_net, Q_evap_skin, Q_solar, Q_rad_sky, Q_rad_bush, Q_rad_vegetation, Q_rad_ground, Q_evap_insulation, success, ntry)
                    end
                end
            end
        end
...
function solve_with_insulation!(T_skin, T_insulation, 
    ...
    ntry = 0
    solct = 0
    solpro = 1   # equivalent to FORTRAN INT(SOLPRO)
    success = true
    Q_gen_net = 0.0u"W"
    ...
    while ntry < 20
        ...
        for i in 1:20
            ...
            (; hc, hd, hd_free) = convection(geometry_out, area_convection, T_air, T_insulation, wind_speed, P_atmos, fluid_type, fO2, fCO2, fN2)

            # Evaporative heat loss
            Q_evap_skin = evaporation(; T_surface=T_skin, wetness=skin_wetness, area=area_convection, hd, eye_fraction, T_air, rh, P_atmos, fO2, fCO2, fN2).Q_evap
            ...
            if conduction_fraction < 1
                ...
                (; T_insulation_calc, T_radiant2) = insulation_radiant_temperature(; body=geometry_out, insulation=insulation_out, insulation_pars, T_core, T_ins_compressed, T_sky, T_lower, T_vegetation, T_bush, T_conduction, area_convection, hc, cd, k_insulation, Q_solar, Q_evap_insulation, Q_rad1, Q_rad2, Q_rad3, Q_rad4, cd1, cd2, cd3, dv1, dv2, dv3, dv4, longwave_depth_fraction, conduction_fraction)
            ...
            T_skin_mean, T_skin_calc1 = mean_skin_temperature(; body=geometry_out, insulation=insulation_out, insulation_pars, Q_env, Q_evap_skin, k_flesh, k_fat, T_core, T_insulation_calc, T_ins_compressed, cd1, cd2, cd3, conduction_fraction)
            ...
            if ΔT_insulation < tolerance_simusol
                ...
                if ΔT_skin < tolerance_simusol
                    ...
                    success = true
                    return return (; T_insulation, T_skin = T_skin_mean, Q_convection, Q_conduction, Q_gen_net, Q_evap_skin, Q_solar, Q_rad_sky, Q_rad_bush, Q_rad_vegetation, Q_rad_ground, Q_evap_insulation, success, ntry)
                else
                ...
                update_T_insulation!(T_insulation, T_insulation_calc, ΔT_insulation, solpro)
            end
            ...
            if solct ≥ 100
                if solpro != 3
                    ...
                    if tolerance_simusol <= 0.001u"K"
                        ...
                        success = false
                        Q_gen_net = 0.0u"W"
                        return (; T_insulation, T_skin, Q_convection, Q_conduction, Q_gen_net, Q_evap_skin, Q_solar, Q_rad_sky, Q_rad_bush, Q_rad_vegetation, Q_rad_ground, Q_evap_insulation, success, ntry)
                    end
                end
            end
        end
